...
export default function Moods() {
  ...
  const makeHappy = () => {
    /* STEP 4 */
    setMood(happyMood)
    document.querySelector('#mood').textContent = happyMood
  };
  const makeSad = () => {
    /* STEP 5 */
    setMood(sadMood)
    document.querySelector('#mood').textContent = sadMood
  };
  const reset = () => {
    /* STEP 6 */
    setMood(initialMood)
    document.querySelector('#mood').textContent = initialMood
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: isHappy ? 'royalblue' : 'crimson', /* STEP 2 */
  };
